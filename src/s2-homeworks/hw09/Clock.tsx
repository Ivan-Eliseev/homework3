...
function Clock() {
  ...
  const start = () => {
    ...
    setTimerId(idTimer as unknown as number)
  ...
  const stringTime = `${date.getHours()}:${normalize(date.getMinutes())}:${normalize(date.getSeconds())}` || <br /> // часы24:минуты:секунды (01:02:03)/(23:02:03)/(24:00:00)/(00:00:01) // пишут студенты
  ...
  const stringDate = `${normalize(date.getDate())}.${normalize(date.getMonth()+1)}.${date.getFullYear()}` || <br /> // день.месяц.год (01.02.2022) // пишут студенты, варианты 01.02.0123/01.02.-123/01.02.12345 не рассматриваем
